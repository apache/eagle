 ...
 */
package eagle.log4j.kafka.hadoop

import java.util.Properties

import eagle.log4j.kafka.Keyer
...
class SecurityLogKeyer(props:Properties) extends Keyer{
   ...
   */
  override def getKey(msg: String): String = SecurityLogKeyer.getKey(msg.split("\\s+"))
}
  ...
  def getKey(fields:Array[String]):String = {
    if (fields.length > 5) {
      if (fields(5) == SUCCESSFUL) {
        return if (fields.length > 7) AuditLogUtils.parseUser(fields(7)) else null
      }
      else if (fields(5) == FAILED) {
        return if (fields.length > 7) AuditLogUtils.parseIP(fields(7)) else null
      }
    }
    null
  }
