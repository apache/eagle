...
import org.apache.eagle.stream.dsl.execution.model.{StreamAppExecution, StreamAppDefinition}

/**
 * The actual function for stream app management
 */
trait StreamAppManager {
  def submit(app:StreamAppDefinition):AppExecutionEntity
  def start(app:StreamAppExecution):Boolean
  def stop(app:StreamAppExecution):Boolean
}

object StreamAppManager{
  def apply():StreamAppManager = new StreamAppManagerImpl()
}