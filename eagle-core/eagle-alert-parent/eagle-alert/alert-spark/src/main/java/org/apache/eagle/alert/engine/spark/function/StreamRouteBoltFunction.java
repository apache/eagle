...
public class StreamRouteBoltFunction implements PairFlatMapFunction<Iterator<Tuple2<Integer, Iterable<PartitionedEvent>>>, Integer, PartitionedEvent> {
    ...
    private Map<StreamPartition, List<StreamRouterSpec>> cachedSRS = new HashMap<>();
    ...
    public Iterator<Tuple2<Integer, PartitionedEvent>> call(Iterator<Tuple2<Integer, Iterable<PartitionedEvent>>> tuple2Iterator) throws Exception {
        ...
        while (events.hasNext()) {
            if (router == null) {
                ...
                Map<StreamPartition, List<StreamRouterSpec>> routeSpecMap = routeState.getRouteSpecMapByPartition(partitionNum);
                ...
                routeCollector = new StreamRouterBoltOutputCollector(routeName, new SparkOutputCollector(), streamContext, routeSpecMap, routePartitionerMap);
                ...
                router.prepare(streamContext, routeCollector, streamWindowMap, streamTimeClockMap, streamSortHandlerMap);
    ...
    public void onStreamRouteBoltSpecChange(RouterSpec spec, Map<String, StreamDefinition> sds, StreamRouterImpl router, StreamRouterBoltOutputCollector routeCollector,
                                            final Map<StreamPartition, StreamSortSpec> cachedSSS, final Map<StreamPartition, List<StreamRouterSpec>> cachedSRS, int partitionNum) {
        //sanityCheck(spec);
