...
public abstract class AbstractHBaseLogReader<T> implements LogReader<T> {
    ...
    public AbstractHBaseLogReader(EntityDefinition ed, List<String> partitions, Date startTime, Date endTime,
        ...
        if (entityDefinition.getPartitions() != null) {
            if (partitions == null || entityDefinition.getPartitions().length != partitions.size()) {
    ...
    public void open() throws IOException {
        ...
        try {
            tbl = EagleConfigFactory.load().getHTable(entityDefinition.getTable());
        ...
        if (this.qualifiers == null) {
            // Filter all
            s1.addFamily(entityDefinition.getColumnFamily().getBytes());
        } else {
            for (byte[] qualifier : qualifiers) {
                s1.addColumn(entityDefinition.getColumnFamily().getBytes(), qualifier);
            }
        }
