...
public class HBaseFilterBuilder {
     ...
     *
     * <p>TODO: For now we don't support one query to query multiple partitions. In future if partition is defined,
            ...
            for (AtomicExpression ae : andExpr.getAtomicExprList()) {
                ...
                if (keyType == TokenType.ID && isTag(fieldName)) {
                    if ((ComparisonOperator.EQUAL.equals(op) || ComparisonOperator.IS.equals(op))
                        ...
                        throw new IllegalArgumentException("Don't support operation: " + op + " on tag field: " + fieldName + ", avaliable options: =, =!, =~, !=~, in, not in, contains, not "
                            + "contains");
                    }
                } else {
                    qualifierFilters.add(entry);
            ...
            if (ComparisonOperator.IN.equals(entry.getOp())
                ...
                if (TokenType.NULL.equals(entry.getValueType())) {
                    if (ComparisonOperator.EQUAL.equals(entry.getOp())
                        ...
                        throw new IllegalArgumentException("Operation: " + entry.getOp() + " with NULL is not supported yet: " + entry.toString() + ", avaliable options: [=, !=, is, is not] "
                            + "null|NULL");
                    }
                } else if (ComparisonOperator.CONTAINS.equals(entry.getOp()) // If [contains, not contains],use SubstringComparator
                    || ComparisonOperator.NOT_CONTAINS.equals(entry.getOp())) {
                    comparable = new SubstringComparator(entry.getValue());
                } else if (ComparisonOperator.LIKE.equals(entry.getOp()) // If [like, not like], use RegexStringComparator
                    ...
                    if (!EagleConfigFactory.load().isCoprocessorEnabled() || type == null || TypedByteArrayComparator.get(type) == null) {
                        comparable = new BinaryComparator(EntityQualifierUtils.toBytes(ed, entry.getKey(), entry.getValue()));
                    } else {
                        comparable = new TypedByteArrayComparator(EntityQualifierUtils.toBytes(ed, entry.getKey(), entry.getValue()), type);
                    }
                }

                SingleColumnValueFilter filter =
                    new SingleColumnValueFilter(ed.getColumnFamily().getBytes(), qualifierName.getBytes(), convertToHBaseCompareOp(entry.getOp()), comparable);
     ...
     * <br/>
     *
     * <p>! Check op must be IN or NOTIN in caller
     *
     * @param entry
     * @return
     */
    private Filter buildListQualifierFilter(QualifierFilterEntity entry) {
        ...
        while (it.hasNext()) {
            String value = it.next();
            BinaryComparator comparator = new BinaryComparator(EntityQualifierUtils.toBytes(ed, fieldName, value));
            SingleColumnValueFilter filter =
                new SingleColumnValueFilter(ed.getColumnFamily().getBytes(), qualifierName.getBytes(), convertToHBaseCompareOp(entry.getOp()), comparator);
            filter.setFilterIfMissing(filterIfMissing);
