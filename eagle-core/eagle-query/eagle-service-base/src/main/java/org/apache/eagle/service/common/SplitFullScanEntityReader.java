 ...
 */
public class SplitFullScanEntityReader<T extends TaggedLogAPIEntity> {
    ...
    public EntityResultSet<T> read() throws Exception {
        final EntityResultSet<T> resultSet = new EntityResultSet<T>(new ArrayBlockingQueue<TaggedLogAPIEntity>(bufferSize));
        ...
        if (size > 0) {
            ...
            for (GenericEntityBatchReader reader : readers) {
                final EntityFetchThread<T> thread = new EntityFetchThread<T>(reader, threadCount, entityCount, resultSet);
                thread.start();
            }
        } else {
