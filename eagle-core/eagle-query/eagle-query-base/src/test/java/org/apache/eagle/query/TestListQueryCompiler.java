...
//		Assert.assertEquals(outputFields.size(), 2);
    ...
    public void testGroupByQueryAggWithoutExpressionInAggFunc() throws Exception {
        String query = "TestLogAPIEntity[@cluster=\"cluster\" AND @datacenter=\"datacenter\" AND EXP{@a + @b} > EXP{@c + @d} AND EXP{@a + @c} < EXP{@b + @d + 0.05}]<@cluster, @datacenter>{sum(@a), " +
            "avg(@b)}";
        ListQueryCompiler compiler = new ListQueryCompiler(query, false);
        ORExpression filter = compiler.getQueryExpression();
        LOG.info(filter.toString());
    ...
    public void testGroupByQueryAggWithExpressionInAggFunc() throws Exception {
        String query = "TestLogAPIEntity[@cluster=\"cluster\" AND @datacenter=\"datacenter\" AND EXP{@a + @b} > EXP{@c + @d} AND EXP{@a + @c} < EXP{@b + @d + 0.07}]<@cluster, @datacenter>{sum" +
            "(EXP{@a+@b+20.0}), avg(EXP{(@a+@c + 2.5)/@d}), count}";
        ListQueryCompiler compiler = new ListQueryCompiler(query, false);
        ORExpression filter = compiler.getQueryExpression();
        LOG.info(filter.toString());
