...
public class TestFlatAggregator {
    ...
    public void testSingleGroupbyFieldSingleFunctionForSummary() {
        ...
        try {
            ...
            Map<List<String>, List<Double>> result = agg.result();
            Assert.assertEquals(result.size(), 2);
            Assert.assertEquals(result.get(Arrays.asList("dc1")).get(0), (double) (entities[0].getNumClusters() + entities[1].getNumClusters() + entities[2].getNumClusters()) + entities[3]
                .getNumClusters(), 0.001);
    ...
    public void testSingleGroupbyFieldMultipleFunctions() {
        ...
        try {
            ...
            Assert.assertEquals(result.get(Arrays.asList("dc1")).get(1), (double) (entities[0].getNumHosts() + entities[1].getNumHosts() + entities[2].getNumHosts() + entities[3].getNumHosts()) +
                entities[4].getNumHosts(), 0.001);
        } catch (Exception ex) {
            LOG.error("Can not aggregate", ex);
            Assert.fail("Can not aggregate");
        }
