...
public class StreamPartition implements Serializable {
    ...
    @Override
    public int hashCode() {
        return new HashCodeBuilder().append(streamId).append(type).append(columns).build();
    }
    ...
    @Override
    public String toString() {
        return String.format("StreamPartition[streamId=%s,type=%s,columns=[%s]]", this.getStreamId(), this.getType(), StringUtils.join(this.getColumns(), ","));
    }
}