...
public class TestMAPRIdResolver {
    ...
    @Test
    public  void testFidToName() throws Exception {
        MAPRIdResolver resolver = new MAPRIdResolver();
        String id = "2050.36.131336";
        try{Assert.assertEquals("/tmp/file.w",resolver.fidToName(id));
        }catch (Exception e){
    ...
    @Test
    public void testNameToFid() throws Exception {
        MAPRIdResolver resolver = new MAPRIdResolver();
        String name = "/tmp/file.w";
        try{
            Assert.assertEquals("2050.36.131336",resolver.nameToFid(name));
        }catch(Exception e){
            System.out.println("Exception is"+ e.getMessage());
        }
    }
