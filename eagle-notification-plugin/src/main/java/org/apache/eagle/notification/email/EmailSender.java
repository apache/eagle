...
/**
 * Email Task worker to Trigger email alert when Eagle Policy  Generates any alert
 */
public class EmailSender implements  Runnable {
    ...
    public EmailSender(EmailContext emailContext){
        this.recipents = emailContext.getRecipients();
        this.configFileName = emailContext.getVelocityTplFile();
        this.subject = emailContext.getSubject();
        this.sender = emailContext.getSender();
        this.cc = emailContext.getCc();
        for(EmailComponent bean : emailContext.getComponents()){
    ...
    public EmailSender(EmailContext alertEmail, ConfigObject eagleProps){
        this(alertEmail);
        this.eagleProps = eagleProps;
    }

    /**
     * Invokes EmailClient and Trigger Email Notification
     */
    @Override
    public void run() {
        int count = 0;
        ...
        while(count++ < MAX_RETRY_COUNT && !success)
        {
            ...
            try {
                ...
                if (eagleProps != null) {
                    ...
                    client = new EmailClient(con);
                }
                else client = new EmailClient();
